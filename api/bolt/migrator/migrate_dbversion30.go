...
func (m *Migrator) migrateDBVersionTo30() error {
	...
	if err := m.updateVolumeResourceControl(); err != nil {
		return err
	}
	return nil
}
...
func (m *Migrator) updateVolumeResourceControl() error {
	...
	for _, endpoint := range endpoints {
		...
		if volumesData, done := snapshot.SnapshotRaw.Volumes.(map[string]interface{}); done {
			...
			findResourcesToUpdate(endpointDockerID, volumesData, toUpdate, volumeResourceControls)
		}
	}
...
func findResourcesToUpdate(dockerID string, volumesData map[string]interface{}, toUpdate map[portainer.ResourceControlID]string, volumeResourceControls map[string]*portainer.ResourceControl) {
